...
psql --set=ON_ERROR_STOP=1 \
  ...
  --set=zelglihof_password="$ZELGLIHOF_DATABASE_PASSWORD" \
	--set=unclet_password="$UNCLET_DATABASE_PASSWORD" \
  --set=jublawoma_password="$JUBLAWOMA_DATABASE_PASSWORD" \
...
CREATE ROLE unclet_web LOGIN PASSWORD :'unclet_password';
CREATE ROLE jublawoma_web LOGIN PASSWORD :'jublawoma_password';
