...
import { publicResourceIds } from "@oliumbi/zelglihof-data/contracts";
...
export const listPublicRecords = createServerFn({ method: "GET" })
	...
	.handler(({ data }) =>
		createPublicRepository(database.sql).list(data.resource, data.page),
	);
export const getPublicRecord = createServerFn({ method: "GET" })
	...
	.handler(({ data }) =>
		createPublicRepository(database.sql).detail(
			data.resource,
			data.value,
			data.bySlug,
		),
