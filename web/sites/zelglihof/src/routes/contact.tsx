            ...
            <Form className="w-full max-w-2xl p-8 flex flex-col gap-4">
                <Field.Root className="flex flex-col gap-0.5">
                    <Select.Root items={types}>
                        <Select.Label className="font-semibold text-sm px-3 text-stone-600">Anfrage</Select.Label>
                        ...
                        <Select.Portal>
                            <Select.Positioner className="outline-hidden select-none z-10" sideOffset={4}>
                                <Select.Popup className="group min-w-[var(--anchor-width)] origin-[var(--transform-origin)] bg-stone-100 border border-stone-950">
                                    <Select.ScrollUpArrow className="">
                                        <ChevronUp/>
                                    </Select.ScrollUpArrow>
                                    <Select.List className="relative">
                                        {types.map(({label, value}) => (
                                            <Select.Item key={label} value={value} className="grid grid-cols-[2rem_1fr] p-0.5">
                ...
                <Field.Root className="flex flex-col gap-0.5">
                    <Field.Label className="font-semibold text-sm px-3 text-stone-600">Anfrage</Field.Label>
                    <Field.Control render={<textarea />} rows={5} className="font-bold border bg-stone-100 border-stone-950 rounded-md px-3 py-2"/>
                </Field.Root>
                ...
                <Button className="text-stone-50 px-3 py-2 bg-amber-700 border border-amber-800 rounded-xl ">Senden</Button>
            </Form>
        </div>
    )
}
