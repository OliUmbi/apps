	...
	return (
		<Field.Root name={name} className="grid gap-2 text-sm">
			...
			<Field.Control
				{...props}
				className={(state) =>
					`w-full rounded-lg border border-bark/20 bg-oat px-3 py-2 focus:outline-2 focus:outline-offset-2 disabled:opacity-50 ${(typeof className === "function" ? className(state) : className) ?? ""}`
				}
			/>
